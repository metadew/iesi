...
public class BigqueryDatasetDeleteCommand implements Runnable {
    @Option(names = {"-n", "--name"}, required = true, description = "the dataset to delete")
    private String datasetName;

    @Override
    public void run() {
        String whichProject = Mount.getInstance().getProjectName("");
        BigqueryConnection.getInstance().deleteDataset(whichProject, datasetName);
    }
}
