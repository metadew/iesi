...
public class ScriptDtoRepository implements IScriptDtoRepository {
    ...
    // @TODO: change select script_version.SCRIPT_VRS_NB to select max(script_version.SCRIPT_VRS_NB) and add GROUP BY script.SCRIPT_ID
    @Override
    public List<ScriptDto> getAll(List<String> expansions, Boolean isLatestOnly) {
        try {
            ...
            String scriptVersion = isLatestOnly ? "max(script_version.SCRIPT_VRS_NB)" : "script_version.SCRIPT_VRS_NB";
            String query = "Select " +
    ...
    public List<ScriptDto> getAllLastVersion(List<String> expansions) {
        try {
            Map<ScriptKey, ScriptDto> scriptDtos = new HashMap<>();
            Map<ActionKey, ActionDto> actionDtos = new HashMap<>();// script + script version + script label UNION script + script version + action + action parameter + expand
            String query = "Select " +
                    "script.SCRIPT_ID, script.SCRIPT_NM, script.SCRIPT_DSC, script.SCRIPT_TYP_NM, max(script_version.SCRIPT_VRS_NB), script_version.SCRIPT_VRS_DSC, 0 INFO_TYPE, " +
                    ...
                    "inner join " + MetadataTablesConfiguration.getInstance().getMetadataTableNameByLabel("ScriptLabels").getName() + " script_label " +
                    "on script.SCRIPT_ID = script_label.SCRIPT_ID and script_version.SCRIPT_VRS_NB = script_label.SCRIPT_VRS_NB " +
                    "group by script.SCRIPT_ID " +
                    getWhereClause(null, null).orElse("") +
                    "union all " +
                    "Select " + "script.SCRIPT_ID, script.SCRIPT_NM, script.SCRIPT_DSC, script.SCRIPT_TYP_NM, max(script_version.SCRIPT_VRS_NB), script_version.SCRIPT_VRS_DSC, 1 INFO_TYPE, " +
