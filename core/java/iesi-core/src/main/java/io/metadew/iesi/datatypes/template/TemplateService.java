    ...
    public boolean matches(DataType dataType, Template template, ExecutionRuntime executionRuntime) {
        if (dataType instanceof DatabaseDatasetImplementation) {
            for (Matcher matcher : template.getMatchers()) {
                ...
                if (!DatabaseDatasetImplementationService.getInstance().getDataItem((DatabaseDatasetImplementation) dataType, matcher.getKey(), executionRuntime)
                        .map(dataItem -> MatcherValueHandler.getInstance().matches(matcher.getMatcherValue(), dataItem, executionRuntime))
                        .orElse(false)) {
                    if (!DatabaseDatasetImplementationService.getInstance().getDataItem((DatabaseDatasetImplementation) dataType, matcher.getKey(), executionRuntime).isPresent()) {
                        log.warn("Dataset " + dataType.toString() + " does not contain item with key " + matcher.getKey());
                    } else {
                        log.warn(DatabaseDatasetImplementationService.getInstance().getDataItem((DatabaseDatasetImplementation) dataType, matcher.getKey(), executionRuntime).get().toString() + " does not match " + matcher.getMatcherValue().toString());
                    }
                    return false;
                }
            }
