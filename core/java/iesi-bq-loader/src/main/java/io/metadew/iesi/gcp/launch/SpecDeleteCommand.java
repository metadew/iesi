...
public class SpecDeleteCommand implements Runnable {
    @Parameters
            ...
            for (BigqueryDatasetSpec bigqueryDatasetSpec : bigquerySpec.getDatasets()) {
                for (BigqueryTableSpec bigqueryTableSpec : bigqueryDatasetSpec.getTables()) {
                    if (bigqueryTableSpec.getName().equalsIgnoreCase("scriptresult")) {
                        BigqueryConnection.getInstance().deleteTable(whichProject, bigqueryDatasetSpec.getName(), new ScriptResultConfiguration());
                    }

                    if (bigqueryTableSpec.getName().equalsIgnoreCase("scriptexecution")) {
                        BigqueryConnection.getInstance().deleteTable(whichProject, bigqueryDatasetSpec.getName(), new ScriptExecutionConfiguration());
                    }
                }

                BigqueryConnection.getInstance().deleteDataset(whichProject, bigqueryDatasetSpec.getName());
            }
