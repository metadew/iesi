...
public class NonRouteScriptExecution extends ScriptExecution {
    ...
    public NonRouteScriptExecution(ScriptVersion scriptVersion, String environment, ExecutionControl executionControl, ExecutionMetrics executionMetrics, Long processId, boolean exitOnCompletion, ScriptExecution parentScriptExecution, Map<String, String> parameters, Map<String, String> impersonations, ActionSelectOperation actionSelectOperation, RootingStrategy rootingStrategy) {
        super(scriptVersion, environment, executionControl, executionMetrics, processId, exitOnCompletion, parentScriptExecution, parameters, impersonations, actionSelectOperation, rootingStrategy);
    }

    @Override
    public void prepareExecution() {
        getExecutionControl().logMessage("script.name=" + this.getScriptVersion().getScript().getName(), Level.INFO);
        getExecutionControl().logMessage("script.version=" + this.getScriptVersion().getNumber(), Level.INFO);
